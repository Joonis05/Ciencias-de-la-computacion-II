...
class BinariaView(BaseView):
    ...
    def _entry_with_label(self, parent, text, default):
        ...
        e = ctk.CTkEntry(parent, width=110, height=34, placeholder_text=default, font=ctk.CTkFont(size=14), justify='center')
    ...
    def _render_structure(self):
        ...
        for idx in range(self._capacity):
            value = self._data[idx] if idx < len(self._data) else 'Vacío'
            ...
            row._default_bg = bg
            row.grid(row=idx // cols, column=idx % cols, padx=5, pady=5, sticky='nsew')
    ...
    def _set_controls_state(self, state):
        ...
        for w in (self._mode_seg, self._key_size_entry, self._size_entry, self._manual_entry, self._btn_structure, self._btn_generate, self._btn_clear, self._btn_save, self._btn_load, self._search_entry, self._btn_search, self._btn_delete, self._btn_reset_search, self._speed_slider):
    ...
    def _begin_binary_animation(self, target):
        ...
        self._status_label.configure(text=f'Buscando {target} mediante búsqueda binaria...')
    ...
    def _step_binary(self, inicio, fin, target, step):
        if not self._is_animating:
            return
        if inicio > fin:
            for r in self._cell_frames[:len(self._data)]:
                r.configure(fg_color=('#FEE2E2', '#450A0A'))
            action = 'Eliminación' if self._pending_action == 'delete' else 'Búsqueda'
            self._status_label.configure(text=f'{action}: {target} no fue encontrado. Intervalo agotado en {step - 1} paso(s).')
        ...
        row.configure(fg_color=('#FB923C', '#9A3412'))
        ...
        if val == target:
            ...
            if self._pending_action == 'delete':
                ...
                self._render_structure()
                self._info_label.configure(text=f'{len(self._data)}/{self._capacity} registros ordenados')
                self._status_label.configure(text=f'{target} encontrado en el índice {medio + 1} y eliminado correctamente.')
            else:
                self._status_label.configure(text=f'Elemento {target} encontrado en el índice {medio + 1}. Paso {step}')
            self._is_animating = False
            self._pending_action = None
            self._set_controls_state('normal')
            return
    ...
    def _on_load(self):
        ...
        if payload is None:
            return
        if payload.get('tipo') != 'binaria' or not isinstance(payload.get('data'), list):
            self._show_error('El archivo no corresponde a una búsqueda binaria válida.')
            return
        try:
            ...
            self._render_structure()
            self._info_label.configure(text=f'{len(data)}/{capacity} registros ordenados')
